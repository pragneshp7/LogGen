...
if EXPONENT <= 0:
	...
	print("LUT1: EXPONENT>0\n")

if MANTISSA <= 0:
	print("ERROR!!!\n")
	quit()
else:
	print("LUT1: MANTISSA>0\n")

if ACCURACY <= 0 or ACCURACY>MANTISSA:
	print("ERROR!!!\n")
	quit()
else:
	print("LUT1:ACCURACY>0 and less equal to than MANTISSA\n")
...
if __name__ == "__main__": 
    ...
    f.write("        case (addr)\n")
    f.write("			8'b0"),
    #f.write("		: log = " + str(SIZE+1) + "'b"),
    f.write("		: log = "),
    one_concat = ''
    one_concat += str(SIZE+1)
    one_concat += '\'b'
    for i in range (0, EXPONENT+1): #to-do
        one_concat += '1'
    for i in range (0,MANTISSA): #to-do
        one_concat += '0'
    one_concat += ';'
    #one_concat += '0000000000;'
    f.write(one_concat)
    f.write("\n")
    #f.write("1"),
    #f.write("0000000000;")
    #f.write("		: log = 19'b1111111110000000000;")
    for i in range (254):
    	if BASE == 'e':
		temp = (np.log(2)/np.log(np.e))*(a-127)
	else:
		temp = (np.log(2)/np.log(int(BASE)))*(a-127)
	num = bin(np.float16(temp).view('H'))[2:].zfill(16)
        #f.write("			8'b{0:b}".format(i+1).zfill(7)),
        #f.write("		: log = 19'b{0};".format(num))
        ...
        a += 1
    f.write("			8'b11111111"),
    #f.write("		: log = " + str(SIZE+1) + "'b0"),
    f.write("		: log = "),
    one_concat = ''
    one_concat += str(SIZE+1)
    one_concat += '\'b0'
    for i in range (0, EXPONENT): #to-do
        one_concat += '1'
    for i in range (0,MANTISSA): #to-do
        one_concat += '0'
    one_concat += ';'
    ...
    f.write(one_concat)
    f.write("\n")
    #for i in range (1, EXPONENT): #to-do
     #   f.write("1"),
    #f.write("0000000000;")
    #f.write("		: log = 19'b0111111110000000000;")
    f.write("        endcase\n")
    f.write("    end\n")
    f.write("endmodule\n")
